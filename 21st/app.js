...
function renderMenuItemCard(item, meta, options = {}) {
  ...
  const comboOffer = showCombo
    ? comboOptions
        .map(
          (combo) =>
            `<p class="menu-item-card__combo-offer">+ ${combo.name} ${formatPrice(combo.price)} · ${combo.description}</p>`
        )
        .join('')
    : '';
  ...
  return `
    <article
      class="menu-item-card${hasPortions ? ' menu-item-card--has-portions' : ''}${showCombo ? ' menu-item-card--has-combo' : ''}${isComboCard ? ' menu-item-card--combo-card' : ''}"
      data-id="${item.id}"
      role="button"
      tabindex="0"
...
function renderModalCombo(item, meta) {
  ...
  container.innerHTML = `
    <p class="item-dialog__portions-label">Transformar em combo:</p>
